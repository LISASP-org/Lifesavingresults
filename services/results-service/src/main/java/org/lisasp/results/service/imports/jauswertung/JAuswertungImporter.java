package org.lisasp.results.service.imports.jauswertung;
...
import org.lisasp.results.service.imports.jauswertung.model.CompetitorType;
import org.lisasp.results.service.imports.jauswertung.model.ValueTypes;
...
public class JAuswertungImporter {
    ...
    private Competition importCompetition(org.lisasp.results.service.imports.jauswertung.model.Competition competition) {
        Event[] events = importEvents(competition.getEvents());

        return new Competition(competition.getName(), "", null, null, events);
    }

    private Event[] importEvents(org.lisasp.results.service.imports.jauswertung.model.Event[] importedEvents) {
        return Arrays.stream(importedEvents).map(e -> toEvent(e)).toArray(Event[]::new);
    }

    private Event toEvent(org.lisasp.results.service.imports.jauswertung.model.Event importetEvent) {
    ...
    private Entry[] importEntries(InputValueType inputValueType, org.lisasp.results.service.imports.jauswertung.model.Entry[] importedEntries) {
    ...
    private Swimmer[] toSwimmers(org.lisasp.results.service.imports.jauswertung.model.Swimmer[] swimmer) {
        if (swimmer == null) {
    ...
    private Penalty[] toPenalties(org.lisasp.results.service.imports.jauswertung.model.Penalty[] penalties) {
        return Arrays.stream(penalties).map(p -> toPenalty(p)).toArray(Penalty[]::new);
    }

    private Penalty toPenalty(org.lisasp.results.service.imports.jauswertung.model.Penalty p) {
        return new Penalty(p.getName(), toPenaltyType(p), p.getPoints());
    }

    private PenaltyType toPenaltyType(org.lisasp.results.service.imports.jauswertung.model.Penalty p) {
    ...
    private static Start toStart(org.lisasp.results.service.imports.jauswertung.model.Entry importedEntry) {
    ...
    private byte toPlace(InputValueType inputValueType, org.lisasp.results.service.imports.jauswertung.model.Entry importedEntry) {
    ...
    private int toTime(InputValueType inputValueType, org.lisasp.results.service.imports.jauswertung.model.Entry importedEntry) {
    ...
    private org.lisasp.results.service.imports.jauswertung.model.Competition deserialize(String json) throws FileFormatException {
        try {
            return objectMapper.readValue(json, org.lisasp.results.service.imports.jauswertung.model.Competition.class);
